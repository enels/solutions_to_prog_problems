...
class PredatoryCreditCard(CreditCard):
    ...
    def charge(self, price):
        ...
        if c_month - s_month == 0 and self._charge_count > self._max_charge_per_month:
            # attracts additional charge of $1
            price += 1
        # entered a new month
        elif c_month - s_month == 1:
            # set the start month to be the same as the current month
            self._start_month = current_month
            # set the charge count to start from 1
            self._charge_count = 1
